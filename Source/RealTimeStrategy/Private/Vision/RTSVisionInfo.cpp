...
void ARTSVisionInfo::ApplyVisionForActor(AActor* Actor, TArray<int32>& CachedTiles)
{
	...
	for (const int32 TileIndex : CachedTiles)
	{
...
void ARTSVisionInfo::NotifyPlayerVisionInfoAvailable()
{
	...
	const ARTSTeamInfo* Team = Player->GetTeamInfo();
                                      ...
                                      int32 ActorLocationHeightLevel)
{
	...
	const int32 CheckedVision = LocalVisionGrid[LocalTileIndex];
	...
	// Check height level.
	const int32 TileHeightLevel = VisionVolume->GetTileHeight(FIntVector(GlobalTileX, GlobalTileY, 0));
	...
	const int32 x1 = ActorLocationTileX;
	const int32 y1 = ActorLocationTileY;

	const int32 dx = abs(x1 - x0), sx = x0 < x1 ? 1 : -1;
	const int32 dy = -abs(y1 - y0), sy = y0 < y1 ? 1 : -1;
	...
	const bool bHasVisionThere = HasLocalVisionAt(x0_, y0_, ActorSightRadiusTile, x0, y0, ActorLocationTileX,
