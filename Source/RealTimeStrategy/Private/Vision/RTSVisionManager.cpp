...
void ARTSVisionManager::Initialize()
{
	...
	// Find local player.
	const APlayerController* FirstLocalPlayerController = GetGameInstance()->GetFirstLocalPlayerController();
	...
	for (TActorIterator<AActor> It(GetWorld()); It; ++It)
	{
		...
		const URTSVisionComponent* VisionComponent = Actor->FindComponentByClass<URTSVisionComponent>();
...
void ARTSVisionManager::Tick(float DeltaSeconds)
{
	...
	for (FRTSVisibleActor& VisibleActor : VisibleActors)
	{
		...
		{
			const ARTSGameMode* GameMode = Cast<ARTSGameMode>(UGameplayStatics::GetGameMode(this));

			// Update server vision.
			for (const ARTSTeamInfo* Team : GameMode->GetTeams())
			{
				...
				{
					const ARTSVisionInfo* TeamVision = VisionInfos[Team->GetTeamIndex()];
					const ERTSVisionState NewVision = TeamVision->GetVision(TileLocation.X, TileLocation.Y);
		...
		if (GetNetMode() != NM_DedicatedServer)
		{
			...
			const ERTSVisionState NewVision = IsValid(LocalVisionInfo)
...
void ARTSVisionManager::AddVisibleActor(AActor* Actor)
{
	...
	const FRTSVisibleActor VisibleActor(Actor);
...
void ARTSVisionManager::AddVisionActor(AActor* Actor)
{
	...
	const FRTSVisionActor VisionActor(Actor);
...
void ARTSVisionManager::UpdateVisionActor(const FRTSVisionActor& VisionActor)
{
	...
	for (const ARTSVisionInfo* VisionInfo : VisionInfos)
	{
	...
	const FVector ActorLocationWorld = VisionActor.Actor->GetActorLocation();
	const FIntVector ActorLocationTile = VisionVolume->WorldToTile(ActorLocationWorld);
	const int32 ActorLocationHeightLevel = VisionVolume->GetTileHeight(ActorLocationTile);
